<template>
	...
	<Card dis-hover :bordered="false">
		...
		<div class="r mt15">
	             	<Page :total="page.totalPage" :page-size="page.pageSize" :current.sync="page.pageNo" @on-change="getMedCommonPickingOrderDetailListClick()" show-total show-elevator />
	             </div>
	</div>  
</template>
<script> 
...
export default {
	...
	methods: {
		// 列表
		async getMedCommonPickingOrderDetailListClick(){
			this.druhList = [];
			this.spinShow = true;
			this.page.userId = this.$router.history.current.query.userId;
			let datalist = await this.$api.druhStorage.getMedCommonPickingOrderDetailList(this.page);
			this.spinShow = false;
  ...
  //       			let datalist = await this.$api.druhStorage.exportAsPrintList(item);
	...
	created(){
		...
		// 列表
		this.getMedCommonPickingOrderDetailListClick();
	},
	watch: { 
