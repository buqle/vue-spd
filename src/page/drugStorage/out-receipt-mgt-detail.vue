<template>
	...
	<Card dis-hover :bordered="false">
	            ...
	            <p slot="extra" class="btn-style">
	                ...
	                <Button type="primary" v-if="comlist.status == 1 && this.page.checkStatus == '0' || comlist.status === 2 && this.page.checkStatus == '0'" @click="confirm()">复核通过</Button> <!-- v-if="this.comlist.status == '1' || this.comlist.status == '9'" -->
	                <Button type="primary" v-if="comlist.type != 4 && comlist.type != 9 && comlist.type != 11 && comlist.status == 1|| comlist.type != 4 && comlist.type != 9 && comlist.type != 11 && comlist.status == 2" @click="confirm1()">生成验收单</Button>
		...
		<TabPane label="已复核" name="1" class="mb10">
			<Table :columns="columns1" :data="druhList" @on-selection-change="handleSelectionChange"></Table> 
			<Spin size="large" fix v-if="spinShow"></Spin>
		             <div class="r mt15">
...
export default {
	...
	methods: {
		handleTab(name){
			...
			// 列表
			this.outStoreDetailListClick(true);
		},
		...
		async outStoreDetailInfoListClick(){
			...
			if(this.$router.history.current.query.state == '0' || this.$router.history.current.query.state == '3'){
				this.nameSh = '1';
			}else{
				this.nameSh = '0';
			}
