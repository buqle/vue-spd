...
export default {
	...
	methods: {
		//  头部 通过不通过
		async handlerProfitAndLossListClick(flg){
			this.spinShow = true;
			let datalist = await this.$api.druhStorage.handlerProfitAndLossList({'checkBillNo':this.$router.history.current.query.id}); 
			if(datalist.data){
				this.spinShow = false;
				this.$router.push({path: "/drugStorage/checkDecrease/afterAdjustment"})
			}else{
