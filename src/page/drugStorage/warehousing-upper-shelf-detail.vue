<template>
	<div class="condition">   
	<Card dis-hover :bordered="false">
	            <p slot="title">
	                单据信息
	            </p>
	            <p slot="extra" class="btn-style">
	                <!-- <Button type="primary" icon="printer" @click="exportAsPrint()">导出execl</Button> -->
	                <Button type="primary" icon="printer" @click="primaryClick()" v-if="nameSh == '0' && comlist.auditStatus == 2 || comlist.auditStatus == 3">确认上架</Button>
	            </p> 
	            <div> 
	                    <Row :gutter="30">
	                        <Col span="6">
	                        配送单/验收单号：{{comlist.distributeCode}}
	                        </Col>
	                        <Col span="6">
	                            状态：{{comlist.statusName}}
	                        </Col>
	                        <Col span="6">
	                            验收时间：{{comlist.receptionTime}}
	                        </Col>
	                    </Row>
	                    <Row :gutter="30" class="mt10"> 
	                        <Col span="6">
	                            上架时间：{{comlist.upUserDate}}
	                        </Col>
	                    </Row>  
	            </div>
	        </Card>
	            <div class="condition-row mb15">  
			<div class="condition-group">
				<Select
			                v-model="goodsName"
			                filterable
			                clearable
			                remote
			                placeholder="请选择药品"
			                :remote-method="queryDrugByListClick"
			                :loading="loading"  style="width: 300px" @on-change="goodsClick(goodsName)">
			                <Option v-for="(option, index) in goodsList" :value="option.ctmmParam" :key="index">{{option.ctmmParam}}</Option>
			            </Select>
			</div>
		</div>
		<Tabs v-model="nameSh" :animated="false" @on-click="handleTab">
		<TabPane label="待上架" name="0" class="mb10">
			<Table :columns="columns" :data="druhList" @on-selection-change="handleListChange"></Table> 
		             <div class="r mt15">
		             	<Page :total="page.totalPage" :page-size="page.pageSize" :current.sync="page.pageNo" @on-change="druhStorageListClick(false)" show-total show-elevator />
		             </div>
		</TabPane>
		<TabPane label="已上架" name="1" class="mb10">
			<Table :columns="columns1" :data="druhList"></Table> 
			<Spin size="large" fix v-if="spinShow"></Spin>
		             <div class="r mt15">
		             	<Page :total="page.totalPage" :page-size="page.pageSize" :current.sync="page.pageNo" @on-change="druhStorageListClick(false)" show-total show-elevator />
		             </div>
		</TabPane> 
		</Tabs> 
	             
	</div>  
</template>
<script> 
export default {
	data () {
	            return { 
	             comlist:'',
	             druhList:[],
	             loading:false,
	             goodsName:'',
	             goodsList:[], 
	             nameSh:'0',
	             spinShow:false,
		page:{
			pageNo:1,
			pageSize:10,
			totalPage:0,
			distributeCode:'',
			hisDrugCodeList:'',
			upFinishType:''
		},
		primaryList:[],
		columns:[
				{
					type: 'selection',
					align: 'center',
					width:'100'
				},
				{
					type: 'index',
					title: '序号',
					align: 'center'
				},
				{ 
					title: '指示货位',
					key: 'actualStore',
					align: 'center',
					render: (h, params) => {
					　　return h('div', [
					　　　　h('Tooltip', {
					　　　　props: { placement: 'top' }
					　　　　}, [
					　　　　h('div', {
					　　　　　　style: {
					　　　　　　　　width: '80px',
					　　　　　　　　overflow: 'hidden',
					　　　　　　　　textOverflow: 'ellipsis',
					　　　　　　　　whiteSpace: 'nowrap',
					　　　　　　},
					　　　　　　}, params.row.actualStore),
					　　　　　　h('div', {
					　　　　　　　　slot: 'content',
					　　　　　　　　style: { whiteSpace: 'normal', wordBreak: 'break-all' }
					　　　　　　},params.row.actualStore)
					　　　　])
					　　])
					}
				},
				{ 
					title: '生产厂家',
					key: 'ctmmManufacturerName',
					align: 'center',
					render: (h, params) => {
					　　return h('div', [
					　　　　h('Tooltip', {
					　　　　props: { placement: 'top' }
					　　　　}, [
					　　　　h('div', {
					　　　　　　style: {
					　　　　　　　　width: '80px',
					　　　　　　　　overflow: 'hidden',
					　　　　　　　　textOverflow: 'ellipsis',
					　　　　　　　　whiteSpace: 'nowrap',
					　　　　　　},
					　　　　　　}, params.row.ctmmManufacturerName),
					　　　　　　h('div', {
					　　　　　　　　slot: 'content',
					　　　　　　　　style: { whiteSpace: 'normal', wordBreak: 'break-all' }
					　　　　　　},params.row.ctmmManufacturerName)
					　　　　])
					　　])
					}
				},
				{ 
					title: '货位类型',
					key: 'storeType',
					align: 'center'
				},
				{ 
					title: '实际货位',
					key: 'realReceiveStore',
					align: 'center',
					render: (h, params) => {
			                              return h('Select', {  
			                                  props:{  
			                                      value: this.druhList[params.index].realReceiveStore,
			                                      transfer:true
			                                  },  
			                                  on: {  
			                                      'on-change':(value,lable) => {
			                                          this.druhList[params.index].realReceiveStore = value;
			                                      }
			                                  },  
			                              },  
			                              params.row.acceptoodsVo.map((obj)=>{
			                                  return h('Option', {
			                                      props: {value: obj.id,label: obj.positionName}  
			                                  });  
			                              })
			                              );  
			                          },  
				},
				{	 
					title: '指示数量',
					key: 'realReceiveQuantiry',
					align: 'center'
				},
				{
					title: '实际上架数量',
					key: 'realNum',
					align: 'center',
					render: (h, params) => {
				                      return h('InputNumber', {
				                          props: {
				                            min:1,
				                            // max:params.index.purchaseQuantity,
				                               type: 'text',
				                               value: this.druhList[params.index].realReceiveQuantiry,
				                               transfer:true
				                          },
				                          on: { 
						            'on-change':(event)=>{
						                                  this.druhList[params.index].realNum = event;
						                              }
						          },
				                      });
				                  }, 
				},
				{
					title: '单位',
					key: 'replanUnit',
					align: 'center'
				},
				{
					title: '药品名称',
					key: 'ctmmTradeName',
					align: 'center',
					render: (h, params) => {
					　　return h('div', [
					　　　　h('Tooltip', {
					　　　　props: { placement: 'top' }
					　　　　}, [
					　　　　h('div', {
					　　　　　　style: {
					　　　　　　　　width: '80px',
					　　　　　　　　overflow: 'hidden',
					　　　　　　　　textOverflow: 'ellipsis',
					　　　　　　　　whiteSpace: 'nowrap',
					　　　　　　},
					　　　　　　}, params.row.ctmmTradeName),
					　　　　　　h('div', {
					　　　　　　　　slot: 'content',
					　　　　　　　　style: { whiteSpace: 'normal', wordBreak: 'break-all' }
					　　　　　　},params.row.ctmmTradeName)
					　　　　])
					　　])
					}
				},
				{
					title: '包装规格',
					key: 'packageSpecification',
					align: 'center'
				},
				{
					title: '生产厂家',
					key: 'ctmmManufacturerName',
					align: 'center',
					render: (h, params) => {
					　　return h('div', [
					　　　　h('Tooltip', {
					　　　　props: { placement: 'top' }
					　　　　}, [
					　　　　h('div', {
					　　　　　　style: {
					　　　　　　　　width: '80px',
					　　　　　　　　overflow: 'hidden',
					　　　　　　　　textOverflow: 'ellipsis',
					　　　　　　　　whiteSpace: 'nowrap',
					　　　　　　},
					　　　　　　}, params.row.ctmmManufacturerName),
					　　　　　　h('div', {
					　　　　　　　　slot: 'content',
					　　　　　　　　style: { whiteSpace: 'normal', wordBreak: 'break-all' }
					　　　　　　},params.row.ctmmManufacturerName)
					　　　　])
					　　])
					}
				},
				{
					title: '生产批号',
					key: 'productBatchNo',
					align: 'center'
				},
				{
					title: '生产日期',
					key: 'realProductTime',
					align: 'center'
				},
				{
					title: '有效期至',
					key: 'realValidEndDate',
					align: 'center'
				},
	                	],
	             columns1:[
				{
					type: 'index',
					title: '序号',
					align: 'center'
				},
				{ 
					title: '指示货位',
					key: 'actualStore',
					align: 'center',
					render: (h, params) => {
					　　return h('div', [
					　　　　h('Tooltip', {
					　　　　props: { placement: 'top' }
					　　　　}, [
					　　　　h('div', {
					　　　　　　style: {
					　　　　　　　　width: '80px',
					　　　　　　　　overflow: 'hidden',
					　　　　　　　　textOverflow: 'ellipsis',
					　　　　　　　　whiteSpace: 'nowrap',
					　　　　　　},
					　　　　　　}, params.row.actualStore),
					　　　　　　h('div', {
					　　　　　　　　slot: 'content',
					　　　　　　　　style: { whiteSpace: 'normal', wordBreak: 'break-all' }
					　　　　　　},params.row.actualStore)
					　　　　])
					　　])
					}
				},
				{ 
					title: '生产厂家',
					key: 'ctmmManufacturerName',
					align: 'center',
					render: (h, params) => {
					　　return h('div', [
					　　　　h('Tooltip', {
					　　　　props: { placement: 'top' }
					　　　　}, [
					　　　　h('div', {
					　　　　　　style: {
					　　　　　　　　width: '80px',
					　　　　　　　　overflow: 'hidden',
					　　　　　　　　textOverflow: 'ellipsis',
					　　　　　　　　whiteSpace: 'nowrap',
					　　　　　　},
					　　　　　　}, params.row.ctmmManufacturerName),
					　　　　　　h('div', {
					　　　　　　　　slot: 'content',
					　　　　　　　　style: { whiteSpace: 'normal', wordBreak: 'break-all' }
					　　　　　　},params.row.ctmmManufacturerName)
					　　　　])
					　　])
					}
				},
				{ 
					title: '货位类型',
					key: 'storeType',
					align: 'center'
				},
				{ 
					title: '实际货位',
					key: 'realReceiveStore',
					align: 'center'
				},
				{	 
					title: '指示数量',
					key: 'realReceiveQuantiry',
					align: 'center'
				},
				{
					title: '实际上架数量',
					key: 'realNum',
					align: 'center',
				},
				{
					title: '单位',
					key: 'replanUnit',
					align: 'center'
				},
				{
					title: '药品名称',
					key: 'ctmmTradeName',
					align: 'center',
					render: (h, params) => {
					　　return h('div', [
					　　　　h('Tooltip', {
					　　　　props: { placement: 'top' }
					　　　　}, [
					　　　　h('div', {
					　　　　　　style: {
					　　　　　　　　width: '80px',
					　　　　　　　　overflow: 'hidden',
					　　　　　　　　textOverflow: 'ellipsis',
					　　　　　　　　whiteSpace: 'nowrap',
					　　　　　　},
					　　　　　　}, params.row.ctmmTradeName),
					　　　　　　h('div', {
					　　　　　　　　slot: 'content',
					　　　　　　　　style: { whiteSpace: 'normal', wordBreak: 'break-all' }
					　　　　　　},params.row.ctmmTradeName)
					　　　　])
					　　])
					}
				},
				{
					title: '包装规格',
					key: 'packageSpecification',
					align: 'center'
				},
				{
					title: '生产厂家',
					key: 'ctmmManufacturerName',
					align: 'center',
					render: (h, params) => {
					　　return h('div', [
					　　　　h('Tooltip', {
					　　　　props: { placement: 'top' }
					　　　　}, [
					　　　　h('div', {
					　　　　　　style: {
					　　　　　　　　width: '80px',
					　　　　　　　　overflow: 'hidden',
					　　　　　　　　textOverflow: 'ellipsis',
					　　　　　　　　whiteSpace: 'nowrap',
					　　　　　　},
					　　　　　　}, params.row.ctmmManufacturerName),
					　　　　　　h('div', {
					　　　　　　　　slot: 'content',
					　　　　　　　　style: { whiteSpace: 'normal', wordBreak: 'break-all' }
					　　　　　　},params.row.ctmmManufacturerName)
					　　　　])
					　　])
					}
				},
				{
					title: '生产批号',
					key: 'productBatchNo',
					align: 'center'
				},
				{
					title: '生产日期',
					key: 'realProductTime',
					align: 'center'
				},
				{
					title: '有效期至',
					key: 'realValidEndDate',
					align: 'center'
				},
	                	],
	            }
	},
	methods: {
		handleListChange(selection) {
			this.primaryList = [];
			for (let i = 0; i < selection.length; i++) {
				let item = {
					id:selection[i].id,
					productBatchNo:selection[i].productBatchNo,
					realNum:selection[i].realNum,
					realReceiveStore:selection[i].realReceiveStore,
					upParentId:selection[i].upParentId,
				}
				this.primaryList.push(item);
				console.log(this.primaryList);
			}
		},
		// 确认上架
		async primaryClick(){
			if(this.primaryList.length == 0){
				this.$Message.error('请勾选商品!');
				return
			}
			let datalist = await this.$api.druhStorage.getCheckAcceptDetailFinishList({'detailListVo':this.primaryList,'distributeCode':this.$router.history.current.query.id});
			if(datalist.code == 200){
				this.shelfInfoTitleListClick();
				this.handleTab(1);
			}
		},
		handleTab(name){
			this.page.upFinishType = name;
			this.shelfInfoListClick(true);
		},
		//  头部 明细
		async shelfInfoTitleListClick(){
			let datalist = await this.$api.druhStorage.shelfInfoTitleList({'distributeCode':this.$router.history.current.query.id}); 
			this.comlist=datalist.data; 
		},
		// 列表
		async shelfInfoListClick(flag){
			if(flag){
				this.page.pageNo = 1;
			}
			this.druhList = [];
			this.page.distributeCode = this.$router.history.current.query.id;
			this.page.upFinishType = this.nameSh;
			// this.page.distributeCode = this.$router.history.current.query.id;
			this.spinShow = true;
			let datalist = await this.$api.druhStorage.shelfInfoList(this.page);
			this.spinShow = false;
			this.druhList=datalist.data.list;
			for (let i = 0; i < this.druhList.length; i++) {
				this.druhList[i].realNum  = this.druhList[i].realReceiveQuantiry;
			}
			this.page.totalPage = datalist.data.count;
			this.page.hisDrugCodeList = '';
		},
		// // 打印
		// async print(){
		// 	window.open(`${this.url}?distributeCode=${this.$router.history.current.query.id}&status=${this.page.status}`, '_blank');
		// },
		// // 导出
		// async exportAsPrint(){ 
		// 	let item ={
		// 		distributeCode:this.$router.history.current.query.id,
		// 		status:this.page.status
		// 	}
  //       			let datalist = await this.$api.druhStorage.exportAsPrintList(item);
		// 	let blob = new Blob([datalist], {type: "application/vnd.ms-excel"});
		// 	let fileName = `${this.getCurrentDate()}配送商详情.xls`;
		// 	let objectUrl = URL.createObjectURL(blob); 
		// 	const link = document.createElement('a')
		// 	link.download = fileName;
		// 	link.href = objectUrl;
		// 	link.click();

		// 	// let objectUrl = URL.createObjectURL(blob); 
		// 	// // window.location.href = objectUrl;
		// 	// window.open(objectUrl);
		// },
		// 药品
		async queryDrugByListClick(query){
			let item = {
				paramName:query,
				queryType:3,
			}
			this.loading = true;
			let datalist = await this.$api.publics.queryDrugByList(item);
			this.loading = false;
			this.goodsList=datalist.data;
		},
		// 药品检索
		goodsClick(name){
			for (let i = 0; i < this.goodsList.length; i++) {
				 if(name == this.goodsList[i].ctmmParam){
					this.page.hisDrugCodeList = this.goodsList[i].bigDrugCode;
				}
			}
			this.shelfInfoListClick();
		},
	}, 
	created(){
		// console.log(this.$router.history.current.query.id);
		// 列表
		this.shelfInfoListClick(false);
		// 明细
		this.shelfInfoTitleListClick();
	},
	watch: { 
	 
	},


}
</script> 
<style scoped>
	.c_green_tit {
        color: #2d8cf0;
    }
</style>
