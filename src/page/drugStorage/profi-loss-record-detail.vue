...
export default {
	data () {
	            return {  
	            	url:`${route}/a/checkbill/print/causticExcessivePrint`,
	             comlist:'',
	             druhList:[],
	             loading:false,
	             goodsName:'',
	...
	methods: { 
		//  头部 明细
		async ExcessiveGetListClick(){
			let datalist = await this.$api.druhStorage.ExcessiveGetList({'causticExcessiveNo':this.$router.history.current.query.causticExcessiveNo}); 
		...
		async ExcessiveListClick(flag){
			...
			this.druhList = [];
			this.page.checkBillNo = this.$router.history.current.query.checkBillNo;
			this.spinShow = true;
		...
		// 打印
		async print(){
			window.open(`${this.url}?causticExcessiveNo=${this.$router.history.current.query.causticExcessiveNo}`, '_blank');
		},
		// // 导出
		// async exportAsPrint(){ 
		// 	let item ={
