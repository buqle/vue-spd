...
export default {
	...
	methods: {
		...
		async auditPassOrNoClick(flg){
			this.spinShow = true;
			let datalist = await this.$api.druhStorage.auditPassOrNoList({'checkBillNo':this.$router.history.current.query.id,'sheveType':1,'type':flg}); 
			if(datalist.data){
				this.spinShow = false;
				this.$router.push({path: "/drugStorage/checkDecrease/inventoryAudit"})
			}else{
		...
		async checkBillGetListClick(){
			let datalist = await this.$api.druhStorage.checkBillGetList({'checkBillNo':this.$router.history.current.query.id,'sheveType':1}); 
