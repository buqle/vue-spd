...
export default {
  data () {
    ...
    columns:[
        ...
        {  
          ...
          align: 'center',
          render: (h, params) => {
          　　return h('div',(params.row.price * params.row.demandQuantity).toFixed(4))
          }
        },
        {  
          title: '批准文号',
  ...
  methods: {
    ...
    // 导出
    async exportAsPrint(){
              let datalist = await this.$api.druhStorage.planExportList({'planCode':this.$router.history.current.query.id});
      let blob = new Blob([datalist], {type: "application/vnd.ms-excel"});
      let fileName = `${this.getCurrentDate()}补货计划单.xls`;
